...
def parse_hand(tiles: str) -> MahjongHand:
    ...
    for char in tiles:
        if char in "123456789":
            ...
            if not current_numbers:
                raise AttributeError(f'No number before family denomination')
            if reading_concealed_kong or reading_declared_group:
                raise AttributeError(f'Cannot designate family inside declared group or concealed kong')
            ...
            for declared_num in declared_group_nums:
                declared = [MahjongTile(number=num, family=family) for num in declared_num]
            ...
            if reading_declared_group or reading_concealed_kong:
                raise AttributeError(f'Winning tile cannot be in a declared group or kong')
            if not current_numbers:
                raise AttributeError(f'Missing current numbers for winning tile')
            if winning_tile or winning_tile_number:
                raise AttributeError(f'Wining tile already set')
            winning_tile_number = current_numbers[-1]
        elif char in '([':
            if reading_declared_group or reading_concealed_kong:
                raise  AttributeError(f'Cannot nest declared groups or concealed kongs')
            if char == '(':
            ...
            if not reading_declared_group:
                raise AttributeError(f'Closing parenthesis without opening parenthesis')
            reading_declared_group = False
            if not (3 <= len(declared_group) <= 4):
                raise AttributeError(f'Declared group must be 3 or 4 tiles, got {declared_group}')
            nums = list(sorted(declared_group))
            if len(nums) == 4 and len(set(nums)) != 1:
                raise AttributeError(f'Exposed kong must be 4 identical tiles, got {nums}')
            if len(set(nums)) == 1 or (len(nums) == 3 and nums[2] - nums[1] == 1 and nums[1] - nums[0] == 1):
            ...
            if not reading_concealed_kong:
                raise AttributeError(f'Closing bracket without opening bracket')
            reading_concealed_kong = False
            if len(concealed_kong) != 4 or len(set(concealed_kong)) != 1:
                raise AttributeError(f'Concealed kong must be 4 identical tiles, got {concealed_kong}')
    ...
    if reading_concealed_kong:
        raise AttributeError(f'Missing closing bracket for concealed kong')
    if reading_declared_group:
        raise AttributeError(f'Missing closing parenthesis for declared group')
